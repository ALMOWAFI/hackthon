...
class AzureMathAnalyzer:
    ...
    def __init__(self, subscription_key=None, endpoint=None):
        ...
        # Azure API credentials - first try direct parameters, then env vars
        self.subscription_key = subscription_key or os.getenv('AZURE_API_KEY', '')
        self.endpoint = endpoint or os.getenv('AZURE_ENDPOINT', '')
        
        print(f"Azure endpoint: {self.endpoint}")
        print(f"Azure key found: {bool(self.subscription_key)}")
